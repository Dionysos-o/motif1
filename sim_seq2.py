...
def gen_seqs(c_seq: np.array, n_seq: int):
    ...
    # print(sum(prob))
    seq_mat = np.random.choice(4, size=(n_seq, k), replace=True, p=prob) + c_seq
    seq_mat = seq_mat % 4
    return seq_mat
...
def gen5():
    ...
    data1_part = gen_seqs(c_seq1, n_seq)
    data2_part = gen_seqs(c_seq2, n_seq)
    data3_part = gen_seqs(c_seq3, n_seq)
    ...
    data1_part_kmers = gen5_com(data1_part)
    label1_kmers = np.zeros(len(data1_part_kmers))

    data2_part_kmers = gen5_com(data2_part)
    label2_kmers = np.zeros(len(data2_part_kmers)) + 1

    data3_part_kmers = gen5_com(data3_part)
    label3_kmers = np.zeros(len(data3_part_kmers)) + 2

    data_random_pick_kmers = gen5_com(data_random_pick)
    label_random_pick_kmers = np.zeros(len(data_random_pick_kmers)) + 3

    data_kmers = np.concatenate((data1_part_kmers, data2_part_kmers, data3_part_kmers, data_random_pick_kmers))
    label_kmers = np.concatenate((label1_kmers, label2_kmers, label3_kmers, label_random_pick_kmers))
